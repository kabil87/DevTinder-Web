...
const EditProfile = ({ user }) => {
  ...
  const handlePhotoSubmit = async (e) => {
    ...
    data.append("cloud_name", "dhhycx2dn");

    const res = await axios.post(
      "https://api.cloudinary.com/v1_1/dhhycx2dn/image/upload",
      data,
    );
    ...
    <div>
      <div className="flex justify-center mb-4">
        <fieldset className="fieldset bg-base-300 border-base-200 rounded-box w-xs border p-4">
          ...
          <input
            type="file"
            className="border border-white p-2 rounded-sm w-40 cursor-pointer"
          ...
          <button className="btn btn-neutral mt-4" onClick={handleUpdate}>
            Update
          </button>
        </fieldset>
      </div>
      ...
      {showerrorToast && (
        <div className="absolute top-0 mt-2 ml-120">
